...
This story is infrastructure configuration. Do NOT:
...
Permitted file changes: `wrangler.toml` (add `[vars]` sections), `.github/workflows/deploy.yml` (CI env var injection required by build-time/runtime strategy), `src/lib/api.ts` (server-side API fetch helper for AC #3/#4), and the temporary env-check page (created then deleted).
...
- `src/lib/api.ts` (added — server-side API fetch helper; scope expanded per code review IG-1)
...
- 2026-03-28: Code review patches — added server-side runtime guard and `PUBLIC_API_URL` undefined check to `src/lib/api.ts` (P-1, P-2); expanded story scope to include `src/lib/api.ts` and `deploy.yml` as permitted changes (IG-1, BS-1).
- 2026-03-28: Code review patches P-3 to P-10 — deploy.yml: URL injection fix (env var instead of inline interpolation), JSON.parse try/catch, empty-URL validation step, formatted JSON output, removed redundant success() guards; api.ts: path leading-slash normalisation, Headers instance spread fix; wrangler.toml: stronger DO NOT DEPLOY warning on [env.production] block.
